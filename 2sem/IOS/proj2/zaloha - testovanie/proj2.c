...
void adult(int A, int AWT, int key)
{
	...
	/*
	int child_number_mutex;
	sem_getvalue(child_enter_mutex, &child_number_mutex);
	*/
	
	//if(child_number_mutex < 3)
	if((memory[0] <= (memory[1] / memory[0])) && ((memory[1] % memory[0]) > 0)) //believe in GOD podmienka :D :D :D :D
	{	
		dprintf(memory[2], "ZDE\n");
		sem_wait(counter_mutex);
		
			memory[3]++;
			dprintf(memory[2], "%d\t: A %d\t: Waiting...\n", memory[3], A);
...
void child(int C, int CWT, int key)
{
	...
	{
		dprintf(memory[2], "%d\t: C %d\t: Waiting...\n", memory[3], C);
	}
	...
	sem_wait(memory1_mutex);
		...
		sem_wait(counter_mutex);
			...
			/*
			int adult_sem_number;
			sem_getvalue(adult_leave_mutex, &adult_sem_number);
			
			dprintf(memory[2], "asm = %d\n", adult_sem_number);
			*/
			
			if((memory[4] < 0) && (memory[1] % 3 == 0))
			{
				sem_post(adult_leave_mutex);
				dprintf(memory[2], "post\n");
			}
