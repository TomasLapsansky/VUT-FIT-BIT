...
void gpu_runVertexPuller(GPUVertexPullerOutput *const              output,
  ...
  //(void)vertexShaderInvocation;
	...
	const GPUVertexPullerHead *heads = puller->heads;
		
	if(heads->enabled != 0) {
		output->attributes[0] = gpu_computeVertexAttributeDataPointer(heads, id);
	} else {
		output->attributes[0] = NULL;
	}
		heads++;
	if(heads->enabled != 0) {
		output->attributes[1] = gpu_computeVertexAttributeDataPointer(heads, id);
	} else {
		output->attributes[1] = NULL;
	}
		heads++;
	if(heads->enabled != 0) {
		output->attributes[2] = gpu_computeVertexAttributeDataPointer(heads, id);
	} else {
		output->attributes[2] = NULL;
	}
		heads++;
	if(heads->enabled != 0) {
		output->attributes[3] = gpu_computeVertexAttributeDataPointer(heads, id);
	} else {
		output->attributes[3] = NULL;
	}
}
...
///  - GPUVertexShaderInput()
	...
	for(VertexShaderInvocation i = 0; i < nofPrimitiveVertices; i++) {
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  float y[3];
  for(int i = 0; i < 3; i++) {
  	x[i] = vertices[i].data[0];
  	y[i] = vertices[i].data[1];
  }
  float x_p = pixelCenter->data[0];
  float y_p = pixelCenter->data[1];
  
  coords->data[0] = ((y[1] - y[2])*(x_p - x[2]) + (x[2] - x[1])*(y_p - y[2]))/((y[1] - y[2])*(x[0] - x[2]) + (x[2] - x[1])*(y[0] - y[2]));
  coords->data[1] = ((y[2] - y[0])*(x_p - x[2]) + (x[0] - x[2])*(y_p - y[2]))/((y[1] - y[2])*(x[0] - x[2]) + (x[2] - x[1])*(y[0] - y[2]));
