...
///  - cpu_setIndexing()
	...
	cpu_createBuffers(phong.gpu, 1, &phong.vertice);
	cpu_createBuffers(phong.gpu, 1, &phong.indice);
	
	cpu_bufferData(phong.gpu, phong.vertice, sizeof(bunnyVertices), bunnyVertices);
	cpu_bufferData(phong.gpu, phong.indice, sizeof(bunnyIndices), bunnyIndices);
	
	cpu_createVertexPullers(phong.gpu, 1, &phong.puller);
	
	cpu_setIndexing(phong.gpu, phong.puller, phong.indice, 2);
	
	cpu_setVertexPullerHead(phong.gpu, phong.puller, 0, phong.vertice, 0, sizeof(VertexIndex) * 3);
	cpu_enableVertexPullerHead(phong.gpu, phong.puller, 0);
	
	cpu_setVertexPullerHead(phong.gpu, phong.puller, 1, phong.vertice, sizeof(float) * 3, sizeof(VertexIndex) * 3);
...
///  - getUniformLocation()
	...
	UniformLocation const projectionLoc = getUniformLocation(phong.gpu, "projectionMatrix");
	UniformLocation const viewLoc = getUniformLocation(phong.gpu, "viewMatrix");
	UniformLocation const cameraLoc = getUniformLocation(phong.gpu, "cameraPosition");
	UniformLocation const lightLoc = getUniformLocation(phong.gpu, "lightPosition");
	
	cpu_uniformMatrix4fv(phong.gpu, projectionLoc, (float*)&projectionMatrix);
	cpu_uniformMatrix4fv(phong.gpu, viewLoc, (float*)&viewMatrix);
	cpu_uniform3f(phong.gpu, lightLoc, (float)phong.lightPosition.data[0], (float)phong.lightPosition.data[1], (float)phong.lightPosition.data[2]);
	cpu_uniform3f(phong.gpu, cameraLoc, (float)cameraPosition.data[0], (float)cameraPosition.data[1], (float)cameraPosition.data[2]);
